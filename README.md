...
- `USE_LOCAL_DATA` (default `false`) – when true, `/files` can read from `v1/data.json` instead of live GitHub (to be wired when that route migrates)
- `DATA_JSON_PATH` defaults to `<repo>/v1/data.json`
